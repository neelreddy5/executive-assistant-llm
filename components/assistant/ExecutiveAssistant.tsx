    ...
    <main className={`assistant-shell ${onboarding ? "preference-mode" : "dashboard-mode"}`}>
      ...
      {onboarding ? (
        <section className="preference-content">
          ...
          <ConversationControls connected={session.connected} connecting={session.status === "connecting"} muted={session.microphoneMuted} onToggleMute={session.toggleMicrophoneMuted} onStop={session.stop} />
